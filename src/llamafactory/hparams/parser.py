...
def get_train_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _TRAIN_CLS:
    ...
    if data_args.packing:
        if (data_args.cutoff_len + 1) % model_args.sequence_parallel_size != 0:
            raise ValueError("cutoff_len must be a multiple of sequence_parallel_size.")
            #data_args.cutoff_len = (data_args.cutoff_len // model_args.sequence_parallel_size + 1) * model_args.sequence_parallel_size
            #print(f"cutoff_len must be a multiple of sequence_parallel_size, reset to {data_args.cutoff_len}.")
    else:
